# README - Meeting Memo Generation

> **Last Updated**: 2026-02-05 by Keming He

Generate meeting memos that capture decisions, actions, and key discussions following project templates.

## Quick Start

Use this skill when you need to:

- Document meeting outcomes and decisions
- Create action item lists with ownership
- Write daily standup summaries

Tell your AI agent: "Create a meeting memo" or "Document this meeting".

## Templates

| Template | Use Case |
| :--- | :--- |
| `memo-template.md` | Standard meeting memos with decisions and actions |
| `standup-template.md` | Daily standup summaries |

## Files

```plaintext
meeting-memo/
├── SKILL.md                    # AI instructions for memo generation
├── README.md                   # This file
└── assets/
    ├── memo-template.md        # Standard meeting memo format
    └── standup-template.md     # Daily standup format
```

## Related

- [meeting-agenda](../meeting-agenda/) - Generate meeting agendas
- [contacts](../contacts/) - Manage contact information

---

> README Template v2.0.0 - KemingHe/common-devx
